"""Renban Sudoku generator.

Strategy: solve a classic grid, then sample orthogonal paths whose cells
already form a consecutive set of distinct digits in the solution. The dig +
TS-grader pipeline checks uniqueness under the full renban constraint.
"""
...
RENBAN_DIFFICULTY_BANDS: dict[str, tuple[float, float, int]] = {
    "easy": (1.0, 2.4, 40),
    "medium": (1.0, 2.4, 50),
    "hard": (1.0, 2.4, 58),
    "expert": (1.0, 2.4, 66),
...
def _is_consecutive_set(
    path: list[tuple[int, int]], solved: list[list[int]]
) -> bool:
    vals = [solved[r][c] for r, c in path]
...
def random_renbans(
    ...
    while len(out) < count and attempts < count * 80:
        ...
        if path is None:
            continue
        if not _is_consecutive_set(path, solved):
            continue
        out.append(
            {
                "id": f"rn{len(out) + 1}",
...
def generate_renban(
...
) -> Iterator[RenbanPuzzle]:
    if difficulty not in RENBAN_DIFFICULTY_BANDS:
        raise ValueError(f"unknown renban band: {difficulty}")
    se_lo, se_hi, max_removals = RENBAN_DIFFICULTY_BANDS[difficulty]
    ...
    while emitted < count:
        ...
        paths = random_renbans(rng, solved, shape)
        ...
        result = grader.grade(puzzle_str, variant="renban", paths=paths)
        ...
        if emitted % progress_every == 0 or emitted == count:
            print(
                f"  [renban/{difficulty}] {emitted}/{count} (attempts: {attempts}, accept: {emitted / attempts:.1%})",
                flush=True,
            )
        base = GeneratedPuzzle(
            id=f"renban-{difficulty}-{seed}-{emitted:04d}",
            variant="renban",
            size=shape.size,
            givens=puzzle_str,
        ...
        yield RenbanPuzzle(base=base, paths=paths)
