"""Anti-King Sudoku generator. See anti_knight.py docstring for the
...
from .constraints.anti_king import KING_DIAGONAL_OFFSETS
...
ANTI_KING_DIFFICULTY_BANDS: dict[str, tuple[float, float, int]] = {
    "easy": (1.5, 2.4, 38),
    "hard": (0.0, 99.0, 54),
    "expert": (0.0, 99.0, 62),
}
...
def generate_anti_king(
    ...
    if difficulty not in ANTI_KING_DIFFICULTY_BANDS:
        raise ValueError(f"unknown anti-king band: {difficulty}")
    se_lo, se_hi, max_removals = ANTI_KING_DIFFICULTY_BANDS[difficulty]
    ...
    while emitted < count:
        attempts += 1
        solved = random_solved(shape, rng, extra_same_offsets=KING_DIAGONAL_OFFSETS)
        dug = dig(
            solved,
            shape,
            rng,
        ...
        result = grader.grade(puzzle_str, variant="anti-king")
        ...
        if emitted % progress_every == 0 or emitted == count:
            print(
                f"  [anti-king/{difficulty}] {emitted}/{count} (attempts: {attempts}, accept: {emitted / attempts:.1%})",
                flush=True,
            )
        yield GeneratedPuzzle(
            id=f"anti-king-{difficulty}-{seed}-{emitted:04d}",
            variant="anti-king",
            size=shape.size,
            givens=puzzle_str,
